...
Use this prompt when spawning each task subagent. Replace `PLAN_FILE_PATH`, `PROGRESS_FILE_PATH`, `USER_RULES`, `STAGE_AND_COMMIT_SCRIPT`, and `APPEND_PROGRESS_SCRIPT` with actual values before spawning. Both script placeholders must be absolute paths. If any placeholder remains unresolved, stop and report the invalid prompt instead of running a command:
...
You run unattended as part of an autonomous plan execution. NOBODY is watching to answer questions. NEVER ask the user anything — do NOT call interactive user input, do NOT pause for input, do NOT stop to request a decision or approval. Asking blocks the entire run indefinitely.
...
- Commit all changes using the script: bash "STAGE_AND_COMMIT_SCRIPT" "feat: <brief task description>" file1 file2 ...
  List all changed files explicitly (source files, test files, plan file)

STEP 4 - LOG PROGRESS (after commit):
Log a header line: bash "APPEND_PROGRESS_SCRIPT" "PROGRESS_FILE_PATH" "task N: <title>"
Then log the details using echo piped to the script:
echo "- modified: <files>
- implemented: <what was done>
- tests: <what tests added, or why skipped>
- validation: <what commands passed>" | bash "APPEND_PROGRESS_SCRIPT" "PROGRESS_FILE_PATH"
...
bash "APPEND_PROGRESS_SCRIPT" "PROGRESS_FILE_PATH" "[decision] task N: <what you decided> — <why: which lint rule / plan intent / convention drove it>"
bash "APPEND_PROGRESS_SCRIPT" "PROGRESS_FILE_PATH" "[deviation] task N: <how the result differs from the plan> — <why>"
